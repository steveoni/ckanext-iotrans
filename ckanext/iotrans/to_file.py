...
class ToFileSpatialHandler:
    ...
    def _get_transform_generator(csv_reader, source_epsg, target_epsg):
        for row in csv_reader:
            ...
            converted_geom = transform_geom(from_epsg(source_epsg),from_epsg(target_epsg),geom)
            yield converted_geom
    ...
    def _create_cached_file(
        ...
        row_genrator = self._get_transform_generator(csv_reader, source_epsg, target_epsg)
        ...
        with fiona.open(
            ...
            crs=from_epsg(target_epsg),
                ...
                outlayer.writerecords(row_genrator)
        return output_filepath

    def to_file(self) -> Dict[str,str]:
        csv_file_path = to_csv_dump(self.params.resource_id, )
        output = {}
        for target_epsg in self.params.target_espgs:
            for target_format in self.params.target_formats:
                filepath = self.create_cached_file(
